...
// @access  Private
router.get("/active", [auth, requireOrganization], async (req, res) => {
  try {
...
router.post(
  ...
  [
    ...
    body("breakType")
      .optional()
      .isIn(["lunch", "coffee", "personal", "meeting", "other"]),
  ],
  async (req, res) => {
    try {
      ...
      // Get the time entry and verify it belongs to user
      ...
      // Add break reference to time entry
...
// @desc    End a break
// @access  Private
router.put("/end/:id", [auth, requireOrganization], async (req, res) => {
  try {
    ...
    const durationMs = breakEntry.endTime - breakEntry.startTime;
    breakEntry.duration = Math.floor(durationMs / 1000);
    ...
    if (timeEntry) {
      ...
      const allBreaks = await Break.find({
        timeEntry: timeEntry._id,
        isActive: false,
      });
      timeEntry.totalBreakTime = allBreaks.reduce(
        (total, brk) => total + brk.duration,
        0
      );
      await timeEntry.save();
    }
...
router.delete("/:id", [auth, requireOrganization], async (req, res) => {
  try {
    ...
    await TimeEntry.findByIdAndUpdate(breakEntry.timeEntry, {
    ...
    const timeEntry = await TimeEntry.findById(breakEntry.timeEntry);
    if (timeEntry) {
      const remainingBreaks = await Break.find({
        timeEntry: timeEntry._id,
        isActive: false,
      });
      timeEntry.totalBreakTime = remainingBreaks.reduce(
        (total, brk) => total + brk.duration,
        0
      );
      await timeEntry.save();
